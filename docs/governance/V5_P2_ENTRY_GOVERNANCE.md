...
The replacement must otherwise preserve the same task class, phase, exact path allowlist, exact dependencies, migration count, and frozen product/security semantics. It may only incorporate compatibility knowledge learned from the unpublished failure. It requires a new historically unique head ref and current-digest owner approval, records both bases and every enabling governance merge, declares zero automated repair rounds and zero human corrective updates, and is the final task in that lineage. Except for the exact Issue #39 environment-preparation resumption and route-export binding correction below, any mismatch, ambiguity, published commit, PR history, predecessor reopening, scope expansion, second replacement, or sibling task returns `HOLD`.
...
The final replacement in Issue #39 is not classified as an implementation failure when direct evidence proves its only failing action was the recorded offline package-lock-only command returning `LOCAL_NPM_CACHE_METADATA_MISS_BEFORE_LOCKFILE_GENERATION` before `package-lock.json` changed or Prisma generation, tests, build, commit, push, or PR occurred. Its remote branch must still be identical to `ec7003b5a55512de3f1380676a336bcd5b8d8b1f`, with zero remote commits or changed paths and no PR history. This narrow classification permits exactly one environment-preparation resumption; it does not create a second replacement or correction.

After the enabling governance PR merges, direct evidence must capture its new `main` twice unchanged and prove that the comparison from `ec7003b5a55512de3f1380676a336bcd5b8d8b1f` contains only that PR's exact governance head commit plus its normal merge commit and the same three protected governance paths. Issue #39 may then be reopened once, revised to bind that stable base while preserving the same branch, 14 paths, exact dependencies, one migration, frozen semantics and zero correction budgets, redigested, and newly approved by the owner. Only after that approval may its branch be fast-forwarded once without force from the old base to the revised base.

Resumed work starts clean and discards the prior uncommitted source/schema/migration draft. Before implementation, it may add only the three frozen exact dependency declarations and run exactly one package-lock-only preparation command frozen by the revised Issue, with lifecycle scripts disabled and public npm access limited to metadata or artifacts required by that dependency graph. Only success enters implementation. Except for the exact recorded route-export binding failure below, a second preparation failure, later implementation or validation failure, changed main, unexpected path or commit, dependency/scope drift, second resumption, sibling task, real email, credential, production/shared database, Provider or deployment returns final `HOLD`. Human review must record `PRE_IMPLEMENTATION_ENVIRONMENT_PREPARATION_RESUMPTION=PASS`; the observer does not infer it.

### One-time Issue #39 Auth.js route-export binding correction

The resumed Issue #39 draft is not classified as a semantic implementation failure when direct evidence proves that dependency preparation succeeded, the complete unpublished local changed-path set still equals its original 14 paths, deterministic offline Prisma generation, `git diff --check`, and lint passed, and the first strict typecheck returned exit `2` solely because `auth.ts` exports `handlers` while `app/api/auth/[...nextauth]/route.ts` directly re-exports nonexistent `GET` and `POST` members. Its remote branch must still equal `45ef653adb6f6856cac173ac570662cefa786981`, with no implementation commit or PR history. This permits one separately owner-gated mechanical binding correction, not a second replacement or ordinary correction budget.

After the enabling governance PR merges, direct evidence must capture its new `main` twice unchanged and prove that `45ef653adb6f6856cac173ac570662cefa786981..new-main` contains only that PR's governance head commit, normal merge commit, and the same three governance paths. Issue #39 may then reopen once more, bind that stable base, preserve the same branch, 14 paths, exact dependencies, one migration, frozen semantics and zero correction budgets, receive a new body digest and owner approval, and fast-forward the same branch once without force.

Work resumes in a new clean worktree. All 14-path draft bytes must remain unchanged except that `app/api/auth/[...nextauth]/route.ts` may replace `export { GET, POST } from "@/auth";` only with an import of the existing `handlers` export and `export const { GET, POST } = handlers;`. Typecheck and every remaining frozen validation command must then run. Any other byte change before validation, second correction, dependency/migration/scope drift, changed main, validation failure, commit/PR anomaly, real email, credential, production/shared database, Provider, UI, UserAssertion, AssetTask, or deployment is final `HOLD`. Human review must record `AUTHJS_ROUTE_HANDLER_EXPORT_BINDING_CORRECTION=PASS`; the observer does not infer it.
