...
The resumed work must start from a clean worktree and must not reuse the abandoned uncommitted source, schema, or migration draft. Before any such implementation file changes, it may add only the three frozen exact dependency declarations to `package.json` and execute exactly one revised-Issue-approved package-lock-only command with lifecycle scripts disabled. That approval must freeze the exact command and limit network access to public npm metadata or artifacts required by the frozen dependency graph; it authorizes no business provider, credential, lifecycle script, or unrelated dependency. Only success begins implementation. Except for the exact recorded route-export binding failure below, a second preparation failure, any later implementation or validation failure, second resumption, correction, second replacement, sibling task, scope drift, force-push, Ready, merge, real email, credential, production/shared database, Provider, or deployment returns final `HOLD`.
...
Issue #39 may receive exactly one `AUTHJS_ROUTE_HANDLER_EXPORT_BINDING_CORRECTION` only because direct evidence records that its successful resumed dependency preparation and implementation remained unpublished, its complete local changed-path set equals the frozen 14 paths, deterministic offline Prisma generation, `git diff --check`, and lint passed, and the first strict typecheck stopped with exit `2` solely on `AUTHJS_ROUTE_HANDLER_EXPORT_SHAPE_TYPECHECK_FAILURE`. Eligibility requires its remote branch to remain exactly `45ef653adb6f6856cac173ac570662cefa786981`, with no implementation commit or PR history, while `auth.ts` exports `handlers` and the route's only incompatible statement is `export { GET, POST } from "@/auth";`.

The enabling control-plane PR must merge first. Its post-merge `main` must be read twice unchanged, and `45ef653adb6f6856cac173ac570662cefa786981..new-main` may contain only that PR's exact governance head commit, its normal merge commit, and exactly `AGENTS.md`, this document, and `docs/governance/V5_P2_ENTRY_GOVERNANCE.md`. Then Issue #39 may reopen once more, preserve the same branch, 14 paths, exact dependencies, one migration, frozen semantics, `maxRepairRounds: 0`, and visible human-correction limit zero, bind the stable new base and new body digest, and receive exactly one current owner approval. Only afterward may the same branch fast-forward once without force from `45ef653adb6f6856cac173ac570662cefa786981` to that base.

Work must resume in a new clean worktree. The preserved 14-path draft may be rematerialized byte-for-byte except that `app/api/auth/[...nextauth]/route.ts` may replace exactly `export { GET, POST } from "@/auth";` with `import { handlers } from "@/auth";` and `export const { GET, POST } = handlers;`. No other implementation byte may change before typecheck and every remaining frozen validation command run. This separately owner-gated mechanical binding action is not an automated repair, ordinary human corrective update, second replacement, sibling task, semantic change, or reusable exception. A changed main, different route edit, any other byte change, second correction, dependency/migration/scope drift, validation failure, commit/PR anomaly, force-push, Ready, merge, real email, credential, production/shared database, Provider, UI, UserAssertion, AssetTask, or deployment returns final `HOLD`. Human review must record `AUTHJS_ROUTE_HANDLER_EXPORT_BINDING_CORRECTION=PASS` before branch alignment or code correction.
...
- Any claimed Issue #39 route-export binding correction without the exact recorded exit-2 diagnostics and passed prerequisite gates; without the unpublished 14-path draft, branch-equals-`45ef653adb6f6856cac173ac570662cefa786981`, and no-PR evidence; without the named governance-only lineage, stable rebound base, new digest, owner approval, and single non-force fast-forward; with any route edit other than the exact `handlers` GET/POST binding; with another implementation byte change before validation; or which is a second correction.
