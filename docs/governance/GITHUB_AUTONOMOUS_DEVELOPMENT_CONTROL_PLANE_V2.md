...
The replacement must preserve the predecessor's authorized base, task class, phase, exact path allowlist, exact dependencies, migration count, and frozen product/security semantics. It may only incorporate compatibility knowledge learned from the failed unpublished attempt. It uses a new historically unique head ref and a new current-digest owner approval, records the predecessor Issue/base/ref plus the failed command, exit code, and failure class, and declares both `maxRepairRounds: 0` and a visible human corrective-update limit of zero. Failure of the replacement is final `HOLD`; no second replacement, sibling task, branch reuse, force-push, predecessor reopening, or published-PR bypass is allowed.
...
- Any claimed pre-publication replacement whose predecessor is not closed `HOLD / not_planned`, whose remote head differs from its authorized base, whose head ref has any PR history, whose predecessor may reopen, whose base/class/phase/paths/dependencies/migration count/semantics differ, whose replacement permits a human correction, whose lineage evidence is ambiguous, or which is a second replacement.
